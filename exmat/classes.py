...
class C_grad:
    ...
    def calc_cval(self,step, C_t):
        ...
        for i in range(len(self.C_i)):
            ...
            for j in range(len(self.C_i)):  # nachbarzellen_c aufaddieren
                ...
                if self.c_ary[j].name == 'lazy' or self.c_ary[j].name=='EPS' :           #produce nothing so no calculation
                    continue
            ...
            if self.c_ary[i].name == 'lazy':     #have their default c
                c_value = c_neighbor
            if self.c_ary[i].name == 'EPS':     #have their default c
                c_value = c_neighbor
            self.C_i[i] = c_value
    ...
    def catcher_c(self,step,C_t,j):
        try:
            ...
            if self.c_ary[j].status == False:
                if self.feedback == 1:
                    conz = self.C_on               #inactive than set to 1
    ...
    def set_c(self,p):
        ...
        if self.c_ary[p].status == False:
            ...
            if self.feedback == 1:
                self.C_i[p] = self.C_on # or inactive than set to 1
