...
C_on=16.0                         #signalconcentration of activ cel#
K =  18.0                         #threshold c#
...
g_ratl=0.05                        #growthrate of radius
g_ratw=0.2                        #growthrate of radius
g_ratm=0.2                        #growthrate of radius
...
def divide(p):        #takes rd point and places new cell
    ...
    if c_ary[p].name=='work':
        c_ary[c_num+1] = cl.Cell(c_num+1, 'work', r_new, stategamble(), xn, yn, zn)
    elif c_ary[p].name=='lone':
        c_ary[c_num+1] = cl.Cell(c_num+1, 'lone', r_new, stategamble(), xn, yn, zn)
    elif c_ary[p].name == 'maybe':
        c_ary[c_num + 1] = cl.Cell(c_num + 1, 'maybe', r_new, stategamble(), xn, yn, zn)
...
def growth(p):
    ...
    if c_ary[p].name=='work':
        if c_ary[p].status:
            c_ary[p].radius = c_ary[p].radius + g_ratw
    if c_ary[p].name=='maybe':
        if c_ary[p].status:
            c_ary[p].radius = c_ary[p].radius + g_ratm
    if c_ary[p].name=='lone':
            c_ary[p].radius = c_ary[p].radius + g_ratl     
...
def pic(a):                     #creats vtu data with cell situation at time step
    ...
    vtk_writer.snapshot("cell_arrangements_lone"+str(a)+".vtu", x_list, y_list, z_list, radii=r_list, x_force=F_x, y_force=F_y,
                        z_force=F_z)
    vtk_writer.writePVD("cell_arrangements_lone"+str(a)+".pvd")
